  ...
  return ch.toUpperCase();
		...
		show: function(elem) {
			...
			inst._mainDiv.stop(true, true).
				css({position: 'absolute', display: 'block', top: '-1000px', width: '355px'});
		...
		_generateHTML: function(inst) {
			...
			for (var i = 0; i < layout.length; i++) {
				...
				for (var j = 0; j < keys.length; j++) {
					...
					if (keyDef) {
						html += (keyDef.action ? '<button type="button" onClick="ring()" class="' +
						...
						html += '<button type="button" onClick="ring()" class="' + this._keyClass +
							(inst.options.useThemeRoller ? ' ui-corner-all ui-state-default' : '') +
